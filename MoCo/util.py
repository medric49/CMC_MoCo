...
def enc_loss_plot(hist, path, record_iter):
    ...
    path = os.path.join(path, 'encoder_loss.png')
    plt.savefig(path)
    plt.close()
...
def cls_loss_plot(hist, path, record_epoch):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Linear Classifier Loss')
    plt.legend(loc=4)
    plt.grid(True)
    plt.tight_layout()

    path = os.path.join(path, 'classifier_loss.png')
    plt.savefig(path)
    plt.close()
...
def accr_plot(hist, path, record_epoch):
    ...
    path = os.path.join(path, 'classifier_accuracy.png')
    plt.savefig(path)
    plt.close()